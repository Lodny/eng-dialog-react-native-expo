...
async function playSound(uri, setSound) {  
  ...
  try {
    const { sound } = await Audio.Sound.createAsync({uri: uri}, {}, checkFinish);  //, { shouldPlay: true }); 
    setSound(sound);
...
export { addDays, getDateString, getDateStringWithDay, playSentence, stopSentence }
